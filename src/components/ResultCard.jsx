...
const ResultCard = ({data}) => {
  ...
  const addToCollection = (item) => {
    const oldData = JSON.parse(localStorage.getItem('myCollection')) || [];
    const newData = [...oldData, item];
    localStorage.setItem('myCollection', JSON.stringify(newData));
      ...
      {data.type === 'Photos' ? (
        <>
          ...
          <div className='absolute top-2 right-2'>
            <button onClick={() => addToCollection(data)}
              className="bg-emerald-700 border border-amber-300 cursor-pointer text-white font-medium px-3 py-2 hover: active:scale-95">Save</button>
          </div>
        </>
      ) : (
        <>
